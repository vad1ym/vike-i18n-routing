...
describe('routes — translated paths', () => {
  it('resolves ru translated path to logical url', () => {
    ...
    expect(i18nRoute.locale).toBe('ru')
    expect(i18nRoute.locales).toEqual(['en', 'ru'])
    expect(i18nRoute.logicalUrl).toBe('/about')
    expect(i18nRoute.params).toEqual({})
    expect(i18nRoute).toMatchObject({
      requestUrl: '/ru/o-nas',
  ...
  it('resolves root path correctly', () => {
    ...
    expect(result.pageContext.locale).toBe('ru')
    expect(result.pageContext.i18nRoute!.logicalUrl).toBe('/')
  })
  ...
  it('falls back to path as logical url when no route match', () => {
    ...
    expect(result.pageContext.i18nRoute!.logicalUrl).toBe('/unknown-page')
  })
...
describe('routes — integration (router + useI18nRoute)', () => {
  it('resolves canonicalUrl, localizes path, and builds alternateUrls', () => {
    ...
    expect(route.alternateUrls).toEqual([
